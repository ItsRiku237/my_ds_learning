...
/* del at position node */
struct node *del_at_pos_node(struct node *tail)
{
    int  pos;
    ...
    printf("%d number node is Deleted : ",pos);

    if(pos == 1 ){
        return del_first_node(tail);
    }
    while (pos>1)
    {
        start = start->next;
        if(start == tail)
            return del_last_node(tail);
        pos--;
    }
